...
| 도구 | 맡는 일 |
| --- | --- |
| iTerm2 | macOS의 창, 탭, 프로필 관리 |
| SSH | 원격 머신까지 연결 |
| Herdr | 프로젝트 작업 공간과 터미널·에이전트 세션 유지 |
| Codex CLI | 코드 분석, 수정, 명령 실행과 테스트 |
| Git | 사람이 검토할 수 있는 변경 이력 관리 |
...
macOS에서는 Homebrew로 Herdr를 설치할 수 있다.
...
Codex CLI는 OpenAI가 안내하는 npm 패키지로 설치한다.
...
`herdr --remote`에서 인증 문제가 나면 Herdr부터 의심하기보다 `ssh evga`가 정상적으로 연결되는지 먼저 확인한다. 암호가 걸린 키를 비대화형 환경에서 사용할 때는 `ssh-agent`에 키가 올라가 있어야 한다.
...
Ubuntu에 있는 프로젝트를 Mac의 Codex가 SSH 명령으로 조작하게 만들 수도 있다. 그러나 그렇게 하면 파일 접근과 명령 실행, 권한, 환경 변수의 경계가 두 머신에 걸친다. 반대로 Ubuntu에서 Codex를 실행하면 그 머신의 컴파일러와 Python 환경, Docker, 테스트 도구를 직접 사용한다.
