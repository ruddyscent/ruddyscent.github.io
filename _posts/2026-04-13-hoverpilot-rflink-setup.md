...
RealFlight Link가 활성화되었고, 포트가 열려 있으며, Windows의 IP 주소도 확인되었다. 이제 macOS에서 `nc` 명령어로 Windows의 TCP 포트 18083에 실제로 연결되는지 확인한다.  터미널에서 다음 명령어를 입력한다.

```bash
nc -vz <Windows_IP_ADDRESS> 18083
```

`nc`는 "netcat"의 줄임말로, 네트워크 연결을 테스트하는 데 사용하는 도구다.  `-v` 옵션은 verbose 모드로, 연결 시도에 대한 자세한 정보를 출력한다.  `-z` 옵션은 포트 스캔 모드로, 실제로 데이터를 전송하지 않고 포트가 열려 있는지만 확인한다.
