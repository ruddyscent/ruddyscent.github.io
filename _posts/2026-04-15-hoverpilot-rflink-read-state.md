...
RealFlight Link는 스트리밍 소켓이 아니다.  단순히 연결만 해서는 상태 데이터를 보내주지 않는다.  TCP로 연결한 뒤, SOAP 형식의 XML 요청을 보내야 그에 대한 응답으로 상태 정보를 받을 수 있다.  즉, 상태를 읽으려면 네 단계를 거쳐야 한다.
...
> SOAP 프로토콜이라고 해서 특별히 더 알아야 할 건 없다. XML을 HTTP 요청에 담아 보내고 응답을 파싱하는 방식으로 이해해도 충분하다.

## 응답 처리 방식

RealFlight Link의 응답은 HTTP 형태로 전달되며 그 안에 XML 형태의 상태 데이터가 들어 있다.  처리 흐름은 이렇다.
...
그래서 파싱된 값을 `FlightAxisState`라는 dataclass로 묶어서 관리하도록 한다.  이렇게 구조화해두면 몇 가지가 편해진다.
...
값이 제대로 들어오는지 확인하는 가장 직관적인 방법은 **값이 실제로 변하는지 확인하는 것**이다.  XML 응답에는 너무 많은 값이 포함되어 있어서 모든 값을 다 출력하면 오히려 확인하기 어려우므로 앞서 본 `state.summary()`가 다음 값만 골라서 보여주도록 했다.
...
즉, **응답은 오지만 실제 데이터가 채워지지 않은 상태**다.  그래서 두 가지 처리를 추가했다.
...
- 이번 글 기준 구현: [rflink-state-read](https://github.com/ruddyscent/hover-pilot/releases/tag/rflink-state-read)
