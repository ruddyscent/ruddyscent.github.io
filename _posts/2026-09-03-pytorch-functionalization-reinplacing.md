...
마지막 `copy_`는 새로 계산한 값을 원래 `x`에 복사합니다. 원래 코드처럼 호출자가 넘긴 `x` 자체에도 변경 결과가 반영되도록 하는 것입니다.
...
이 파이썬 함수는 변환을 감싸는 진입점입니다. 실제 변환은 PyTorch 내부의 텐서 연산 계층인 ATen에서 이루어집니다. 파이썬 코드의 문장을 직접 고치는 대신 실행되는 텐서 연산을 바꿉니다. 예를 들어 입력을 직접 바꾸는 `aten.add_`를 새 결과를 반환하는 `aten.add`로 바꿉니다.

이 표현에서 각 노드는 대체로 "입력을 받아 새 출력을 만든다"는 형태입니다. 덕분에 컴파일러가 연산 사이의 데이터 의존성을 파악하기 쉬워집니다. 공통 부분식과 사용되지 않는 값을 제거하고 연산을 재배치할 수도 있습니다. 여러 연산을 하나의 커널로 합치는 연산 융합(fusion), 자동 미분, 그래프 변환도 다루기 수월합니다.
...
이슈 #195285의 예에서는 입력이 `float64` 51,520개였으므로 한 번 복제하는 데이터의 크기는 다음과 같습니다.

```text
51,520 × 8 bytes = 412,160 bytes
```

이 복사가 매 호출마다 발생했습니다. 작은 산술 연산에서는 계산보다 메모리를 읽고 쓰는 비용이 더 커질 수 있습니다.

그렇다면 처음부터 제자리 변경을 유지하면 되지 않을까? 항상 그렇지는 않습니다. 함수형 `slice_scatter`가 앞뒤 연산과 융합되어 별도 중간 결과를 만들지 않는다면 제자리 변경보다 오히려 유리할 수 있습니다. 제자리 변경에는 텐서 데이터를 담는 메모리 공간인 버퍼(buffer)가 필요합니다. 이 때문에 중간 결과를 실제 메모리에 저장(materialize)하도록 강제하고 연산 융합의 선택지를 줄이기도 합니다.
...
결과가 달라집니다. 제자리 연산 복원에는 이전 값이 이후에도 사용되는지 확인하는 생존성(liveness) 분석과 같은 메모리를 공유하는 텐서를 추적하는 별칭 분석이 필요합니다.
...
- 최종 결과가 정말 같은 그래프 입력으로 다시 복사되는가?
...
이 절부터 살펴볼 기존 구현은 PR의 기준 커밋 `e4b9ce8`, 개선안은 PR 커밋 `c0ed45b`에 고정했습니다. 작성 시점에 개선안은 아직 병합되지 않은 초안이므로 기존 구현과 구분해서 읽으면 좋습니다.
...
2. 방금 갱신된 `field`에서 일부 값을 읽어 `indices` 위치에 다시 씁니다.
...
기존 수익성 판단은 다음처럼 직접 연결된 형태를 인식했습니다.

```text
generalized_scatter ─▶ copy_(original_input, ...)
```

그러나 실제 그래프에서는 중간에 `index_put`이 있었습니다.

```text
generalized_scatter ─▶ index_put ─▶ copy_(original_input, ...)
```

그래서 안전성 분석에서는 제자리 연산 복원이 가능하다고 판단할 수 있었지만 그 단계에 도달하기 전에 수익성 판정 조건을 통과하지 못해 후보에서 제외됐습니다. 결과적으로 전체 버퍼 복제가 남았습니다.
...
하지만 실제 그래프에서 `generalized_scatter`의 결과는 두 곳에서 사용됩니다.
...
`values`가 갱신된 `field`에서 계산되기 때문에 `scatter.users`의 개수는 하나가 아닙니다. “결과를 사용하는 연산이 여러 개다”라는 이유만으로 최적화 후보에서 제외하면 실제 문제를 해결하지 못합니다.
...
여기서 첫 번째 인자인 `base`가 실제 갱신 대상입니다. 그래프 탐색은 결과를 사용하는 모든 연산을 따라가지 않고 그 결과가 다음 연산의 갱신 대상 텐서로 쓰이는 연결만 따라갑니다. 아래 코드의 `user`는 현재 결과를 사용하는 연산을 뜻하며 `user.args[0] is current`가 이 연결을 확인하는 조건입니다.
...
이슈 #195285의 첫 접근이 이 문제를 보여 줍니다. 값을 읽는 추가 연산이 있지만 그 존재만으로 불법인 것은 아닙니다.
...
Inductor는 입력과 출력이 어차피 실제 메모리에 저장될 상황이나 결과가 결국 원래 입력으로 다시 복사되는 상황을 제자리 변경이 수익성 있는 신호로 사용합니다. 완벽한 성능 예측 모델이라기보다 제한된 정보로 내리는 보수적인 경험적 판단 기준입니다.

이 판단 순서는 [PR의 `should_reinplace_scatter()`](https://github.com/pytorch/pytorch/blob/c0ed45b28595c6e30e37a211c14cc63228227ef9/torch/_inductor/fx_passes/reinplace.py#L261-L289)에서 읽을 수 있습니다. 이미 실제 메모리에 만들어지는 경우와 원래 입력으로 다시 복사하는 경로가 있는 경우에는 `True`를 반환합니다. 나머지는 연산 융합이 유리할 것으로 보고 함수형 표현을 유지합니다.
...
최종 값이 맞더라도 선택한 위치에 값을 반영하는 함수형 연산(scatter)이 비효율적으로 남을 수 있습니다. 그래서 컴파일러가 프로그램을 분석하고 변환할 때 사용하는 중간 표현(IR)도 확인합니다. 단순히 `aten.clone` 문자열 개수만 검사하면 다른 최적화 단계에서 복제 연산을 변형하거나 제거했을 때 테스트가 잘못 통과할 수 있습니다. 문제를 대표하는 중간 표현인 `slice_scatter`가 없는지도 함께 검사하면 회귀 테스트의 의도를 더 정확하게 표현할 수 있습니다.

최적화가 적용되면 안 되는 경우도 검증해야 합니다.

- 결과가 원래 입력이 아닌 다른 입력으로 다시 복사되는 경우
...
`TORCH_LOGS`는 PyTorch에서 출력할 로그를 선택하는 환경 변수입니다. `2>`는 표준 오류로 출력되는 로그를 `torch-compile.log` 파일에 저장합니다. 같은 이름의 파일이 있으면 덮어쓰므로 이전 결과와 비교하려면 파일명을 다르게 지정합니다.

각 설정은 서로 다른 처리 단계의 출력을 보여 줍니다.

- `aot_graphs`: 함수형 변환을 거쳐 Inductor에 전달되는 연산 그래프
- `post_grad_graphs`: Inductor의 그래프 최적화 이후 남은 연산
- `output_code`: 최종 생성된 실행 코드

`torch-compile.log`를 편집기로 열고 `slice_scatter`, `aten.clone`, `index_put`, `copy_`를 검색해 보면 됩니다.

로그 파일에는 최적화 전후의 출력이 함께 들어 있습니다. `aten.clone`이나 `slice_scatter`가 검색된다는 사실만으로 최적화에 실패했다고 판단하면 안 됩니다. 로그 줄의 `[__aot_graphs]`, `[__post_grad_graphs]`, `[__output_code]` 표식을 확인해 어느 단계의 출력인지 구분합니다. 특히 `post_grad_graphs`에서 복제나 함수형 갱신 연산이 남는지 보고 `output_code`에서는 최종 실행 코드가 어떻게 만들어졌는지 확인하면 됩니다.

이 로그는 주로 컴파일할 때 출력됩니다. 이미 만들어진 실행 코드를 재사용하는 호출에서는 같은 로그가 반복되지 않을 수 있습니다. 각 로그 설정의 의미는 [PyTorch의 로그 등록 코드](https://github.com/pytorch/pytorch/blob/e4b9ce80a342100d49acab38bb4bf7ba0bf87118/torch/_logging/_registrations.py#L106-L129)에서도 확인할 수 있습니다.
...
1. `x[1:-1].add_(delta)`가 왜 단순한 `add`가 아니라 선택한 위치에 값을 반영하는 연산 계열의 표현을 필요로 하는가?
2. 뷰의 크기(`size`), 각 차원에서 다음 원소까지의 간격(`stride`), 저장 공간에서 시작 위치까지의 거리(`storage_offset`) 가운데 별칭 판단에 각각 어떤 정보가 필요한가?
3. `index_put(..., accumulate=True)`도 같은 제자리 연산 복원 규칙을 적용할 수 있는가?
4. 같은 위치를 여러 번 지정할 때 CPU와 GPU의 실행 순서 및 결정성은 어떻게 달라지는가?
5. 그래프 입력과 중간 텐서에 서로 다른 제자리 연산 복원 규칙이 필요한 이유는 무엇인가?
6. 선택한 위치에 값을 반영하는 함수형 연산이 융합되는 경우와 버퍼의 제자리 변경이 유리한 경우를, 실행 전에 비용을 추정하는 방식으로 구분할 수 있는가?
7. 허용 목록에 지정한 위치의 값을 갱신하는 연산을 추가하려면 어떤 조건에서 원래 동작이 보존되는지와 어떤 경우에 최적화를 거부해야 하는지를 검증해야 하는가?
8. 경로 인식과 생존성 분석 사이의 책임 경계를 어디에 두는 것이 유지보수에 유리한가?
