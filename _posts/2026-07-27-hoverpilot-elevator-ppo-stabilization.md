---
layout: post
title: HoverPilot - elevator 하나로 PPO 호버 드리프트 잡기
...
그래서 제어 채널을 elevator 하나로 줄였다. 이 글은 그 좁은 문제 안에서 PPO가 실제로 호버링에 성공하기까지, 무엇을 관찰하고 어떤 가설을 어떻게 고쳐 나갔는지에 대한 기록이다. 핵심은 두 가지다. 하나는 관측의 부호를 물리적으로 일관되게 맞추는 일이고, 다른 하나는 학습해야 할 자유도를 정책 구조 자체로 줄이는 일이었다.
