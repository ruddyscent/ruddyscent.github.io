...
2편에서는 정이십면체를 세분해 삼각형 주 격자와 오각형·육각형 쌍대 격자를 만들고 방향 있는 모서리의 순환으로 $\nabla\times$를 계산하는 방법을 정했습니다. 이제 이 공간 연산자를 시간에 따라 움직여야 합니다. 이번 편의 질문은 **네 전자기장 성분을 어디에 놓고 어떤 순서로 갱신해야 파동이 구면을 따라 전파하는가**다.
...
![TM 셀(육각형·오각형)은 중심에 방사 전기장, 모서리에 접선 자기장을 두고, TE 셀(삼각형)은 중심에 방사 자기장, 모서리에 접선 전기장을 둡니다.](/assets/img/ionosphere-fdtd/tm-te-cell-geometry.svg)
...
한 스텝은 결국 네 개의 갱신으로 이뤄집니다. 각각은 맥스웰 방정식의 두 법칙(패러데이 전자기 유도 법칙과 앙페르·맥스웰 법칙)을 격자 위에서 **적분 형태**로 쓴 다음, 그에 딸린 길이나 면적으로 나눈 것입니다. 복잡한 수식 대신, 각 갱신이 무엇을 무엇으로 미는지만 그림과 함께 짚어 봅니다.

**① 접선 자기장 $H_t$** — 모서리를 사이에 둔 두 지점의 $E_r$ 차이(표면 방향 기울기)와 위아래 평면에 놓인 $E_t$의 차이(방사 방향 변화)를 합쳐 $H_t$를 전진시킵니다.

![주–쌍대 모서리 교차점에서 접선 자기장을 갱신하는 스텐실. 모서리 양 끝의 방사 전기장과 위아래 평면의 접선 전기장을 입력으로 씁니다.](/assets/img/ionosphere-fdtd/update-ht-stencil.svg)

**② 방사 자기장 $H_r$** — 삼각형 하나의 세 모서리를 따라 $E_t$를 한 바퀴 돌며 더한 값(순환)을 삼각형 넓이로 나눕니다. 딱 패러데이 법칙("자기장의 변화는 둘레를 도는 전기장의 순환") 그대로입니다.

![주 삼각형 중심에서 방사 자기장을 갱신하는 스텐실. 삼각형 세 모서리의 접선 전기장 순환을 넓이로 나눕니다.](/assets/img/ionosphere-fdtd/update-hr-stencil.svg)

**③ 방사 전기장 $E_r$** — 이번엔 앙페르 법칙입니다. 쌍대 셀(오각형 또는 육각형)의 둘레를 따라 $H_t$를 한 바퀴 돌아 더하고 셀 넓이로 나눕니다. 파원 전류 $J_r$는 바로 이 자리에서 더해집니다.

![쌍대 셀 중심에서 방사 전기장을 갱신하는 스텐실. 육각형(또는 오각형) 둘레의 접선 자기장 순환을 넓이로 나누고, 같은 위치의 파원 전류를 뺍니다.](/assets/img/ionosphere-fdtd/update-er-stencil.svg)

**④ 접선 전기장 $E_t$** — 모서리를 가로지르는 두 삼각형 중심의 $H_r$ 차이에서 위아래 평면의 $H_t$ 차이를 뺀 값으로 $E_t$를 전진시킵니다.

![주–쌍대 모서리 교차점에서 접선 전기장을 갱신하는 스텐실. 좌우 삼각형 중심의 방사 자기장과 위아래 평면의 접선 자기장을 입력으로 씁니다.](/assets/img/ionosphere-fdtd/update-et-stencil.svg)
...
다음 **4편**에서는 이 네 개의 갱신식을 NumPy로 옮깁니다. 특히 오각형과 육각형의 순환을 파이썬 반복문 없이 한 번에 계산하게 해 주는 접속 표(incidence table)를 이용한 벡터화 기법을 다룹니다.
