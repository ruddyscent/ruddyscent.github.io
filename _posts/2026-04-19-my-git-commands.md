...
처음 Git 환경을 잡을 때 가장 먼저 넣어 두는 설정들입니다.
...
커밋 전에 무엇이 들어가는지 확인하는 습관은 생각보다 중요합니다.
...
브랜치를 확인하고 이동하거나 새로 만들 때 가장 많이 쓰는 명령어들입니다.
...
`git branch`는 로컬 브랜치 목록, `git branch -r`는 원격 브랜치 목록을 보여 줍니다. 브랜치 이동 목적이라면 `checkout`보다 `switch`가 의도가 더 명확해서 개인적으로 더 자주 쓰고 있습니다.

## 6. 원격 저장소

원격 저장소 주소를 확인하거나 연결을 바꿀 때 사용합니다.
...
저장소를 처음 연결할 때는 `git remote add origin ...`을 쓰고, HTTPS에서 SSH로 바꾸거나 저장소 주소가 변경됐을 때는 `git remote set-url origin ...`이 필요합니다.

## 7. 푸시

로컬 커밋을 원격 저장소에 올릴 때 사용합니다.
...
원격 변경 사항을 가져오면서 내 작업을 다시 정리할 때 씁니다.
...
충돌이 발생하면 수정한 뒤 `git rebase --continue`로 이어서 진행합니다. merge보다 rebase를 선호한다면 자주 만나게 되는 조합입니다.

## 9. cherry-pick

특정 커밋 하나만 다른 브랜치로 가져오고 싶을 때 유용합니다.
...
기본 브랜치 이름을 바꾸는 상황에서 주로 사용합니다.

```bash
git branch -M main
```

예전에는 `master`를 기본 브랜치로 쓰는 저장소가 많았지만 지금은 `main`으로 맞추는 경우가 많습니다.
...
태그를 만들고 원격에 올려 두면 특정 배포 시점을 다시 찾기 쉬워집니다. 잘못 만든 태그는 로컬과 원격에서 각각 삭제해 주면 됩니다.
