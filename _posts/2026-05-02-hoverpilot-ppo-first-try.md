...
subtitle: Hover 환경 위에 PPO를 올릴 때 필요했던 정책 구조, 연속 action 처리, rollout 수집
tags:
- hoverpilot
...
[이전 글](/2026-04-18-hoverpilot-gymnasium-interface/)에서는 HoverPilot을 Gymnasium 환경과 호환되는 형태로 맞추었다. `reset()`과 `step()`이 있고, observation과 reward, termination도 연결되어 있다. 하지만 그 상태만으로는 아직 "PPO가 돌아간다"고 말하기는 어렵다.

실제로 학습 루프를 만들려면 여기에 **정책 네트워크**, **연속 action 샘플링**, **rollout 수집**, **advantage 계산**, **정책 업데이트**가 차례로 연결되어야 한다. 이번 글에서는 HoverPilot 관점에서 PPO가 어떤 부품들로 이루어져 있고, 각 부품이 학습 루프 안에서 어떤 역할을 하는지 정리한다.
...
![HoverPilot 관점에서 본 PPO 학습 루프 전체 흐름. RealFlight Hover Trainer의 observation에서 시작해 Actor-Critic 정책, 연속 action 샘플링, rollout 수집, GAE advantage 계산, PPO clipped update, 정책 업데이트로 이어지는 7단계 다이어그램](/assets/img/hover-pilot/ppo-learning-loop-lineart.png)
...
한 줄로 말하면, **조종사(actor)와 평가자(critic)가 한 몸에 들어 있는 구조**다. observation이 들어오면 actor는 "이 상황에서 어떤 입력을 낼까"를 결정하고, critic은 "지금 상황이 얼마나 괜찮은가"를 점수(value)로 매긴다.

HoverPilot에서는 이 둘을 같은 네트워크 안에 두고, 앞쪽의 shared MLP는 함께 쓴다. 그 뒤에서 한 갈래는 action 분포를 만드는 actor로, 다른 갈래는 상태 가치를 추정하는 critic으로 나뉜다.

![HoverPilot의 Actor-Critic 네트워크 구조. 12차원 observation이 shared MLP를 거쳐 Actor와 Critic으로 갈라지고, Actor는 Gaussian policy와 네 개의 조종 입력을 만들며 Critic은 상태 가치 V(s)를 추정한다.](/assets/img/hover-pilot/actor-critic-structure-lineart.png)

_HoverPilot의 Actor-Critic 구조. observation에서 공통 표현을 만든 뒤, actor는 조종 입력 분포를 만들고 critic은 현재 상태의 value를 추정한다._
...
샘플링된 action은 그대로 쓰지 않고, RealFlight 쪽에서 허용하는 action space 범위 안으로 clip해서 적용한다. 아래 그림에서 중요한 흐름은 `mean/std`에서 action을 뽑고, 네 조종 채널로 나눈 뒤, 실제 환경에 넣기 전에 범위를 맞춘다는 점이다.

![HoverPilot에서 연속 action을 다루는 흐름. Actor가 mean과 std를 출력하면 Gaussian policy에서 action을 샘플링하고, aileron, elevator, throttle, rudder 네 조종 채널로 나눈 뒤 action space 범위로 clip해서 RealFlight에 적용한다. log_prob는 나중에 새 정책과 이전 정책을 비교하기 위해 저장한다.](/assets/img/hover-pilot/continuous-action-sampling-lineart.png)

_연속 action을 한 점으로 고정하지 않고 분포에서 샘플링하면, 초기 학습 중에도 다양한 조종 입력을 탐색할 수 있다. 단, 실제 적용 전에는 환경이 허용하는 범위 안으로 clip한다._

또 하나 저장해야 할 값이 `log_prob`다. PPO는 업데이트할 때 "rollout을 모을 당시의 정책이 이 action을 낼 확률"과 "현재 정책이 같은 action을 낼 확률"을 비교한다. 그래서 action 값만 저장하면 부족하고, 그 action이 당시 정책에서 얼마나 그럴듯했는지도 함께 저장해야 한다.
...
![HoverPilot PPO에서 rollout을 수집하는 흐름. 현재 정책으로 RealFlight step을 반복하면서 obs, action, reward, done, value, log_prob를 timestep별로 모아 rollout buffer에 저장하고, done=True가 나오면 reset하며, 모은 최근 경험은 업데이트에 사용한 뒤 버린다.](/assets/img/hover-pilot/rollout-buffer-lineart.png)

_PPO는 오래된 경험을 계속 재사용하기보다, 현재 정책으로 방금 모은 짧은 rollout을 업데이트에 사용한다. 그래서 각 step의 action뿐 아니라 value와 log_prob도 함께 저장해야 한다._

rollout buffer에는 매 스텝의 observation, action, reward, episode가 끝났는지 표시하는 `done`, critic의 value 추정값, 그리고 그때의 `log_prob`를 같이 담는다. 여기서 `done`은 value bootstrap을 끊는 기준이 된다. episode가 끝난 시점부터는 다음 state의 value를 이어서 쓰면 안 되기 때문이다.
...
![HoverPilot PPO에서 advantage를 계산하는 직관. rollout timeline의 reward와 critic의 value V(s)를 비교해 실제 결과가 예상보다 좋으면 A > 0, 예상보다 나쁘면 A < 0으로 보고, GAE는 이후 reward와 다음 value를 섞어 actor에게 줄 학습 신호를 만든다.](/assets/img/hover-pilot/gae-advantage-lineart.png)

_advantage는 critic의 예상보다 실제 결과가 얼마나 좋거나 나빴는지를 나타낸다. GAE는 한 순간의 reward만 보지 않고 이후 흐름과 value bootstrap을 함께 섞어 이 신호를 조금 더 안정적으로 만든다._
...
업데이트할 때는 rollout을 모을 당시의 `log_prob`와 현재 정책에서 다시 계산한 `log_prob`를 비교한다. 이 비율이 `ratio`다. `ratio`가 1에 가까우면 정책이 거의 그대로라는 뜻이고, 1보다 크면 그 action을 더 자주 내는 방향으로 바뀌었다는 뜻이다.

여기에 advantage를 곱하면 학습 방향이 정해진다. `A > 0`이면 그 action의 확률을 높이고, `A < 0`이면 낮춘다. PPO의 핵심은 이 변화가 너무 커지지 않도록 `ratio`를 `1 ± clip_epsilon` 범위에서 제한하는 데 있다.
