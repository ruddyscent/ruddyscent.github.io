...
코딩테스트에서 정렬 문제는 자주 나오지만, 조건이 하나 더 붙는 순간 당황한다. 예를 들어, "특정 인덱스를 기준으로 정렬하고, 같으면 사전순으로 정렬하라" 같은 문제가 그렇다. C#에서 이런 **다중 정렬(Multi-Level Sorting)**을 어떻게 처리하는지 알아보자.
...
정렬의 내부 동작을 직접 제어하는 방법이다. `Array.Sort` 메서드에 커스텀 비교 함수를 전달해 원하는 정렬 기준을 구현한다.
...
## 방법 2: LINQ OrderBy
위 방법보다 가독성이 좋아 실수를 줄이기 쉽다. LINQ의 `OrderBy`와 `ThenBy`를 활용하면 정렬 기준이 명확하게 드러난다.
...
`OrderBy`로 첫 번째 기준을 정렬하고 나서 값이 같으면 `ThenBy`로 두 번째 기준을 적용한다. LINQ에서는 이렇게 정렬 기준을 “순서대로 선언”해서 다중 정렬을 구현한다.
...
기본은 오름차순 정렬이지만 내림차순이 필요하다면 `OrderByDescending`과 `ThenByDescending`을 사용한다.
