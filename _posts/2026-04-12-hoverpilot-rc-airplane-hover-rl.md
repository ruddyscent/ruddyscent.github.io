...
RealFlight는 RC 비행 시뮬레이터 중에서도 가장 널리 사용되는 상용 소프트웨어로, 정밀한 물리 엔진과 실제와 유사한 조종 감각을 제공한다. 무엇보다 **RealFlight Link**라는 통신 인터페이스를 제공하여, 시뮬레이터 내부 상태를 외부 프로그램에서 읽고, 조종 입력을 다시 전달할 수 있다.  이 기능 덕분에 RealFlight는 단순한 훈련 도구를 넘어, **강화학습 환경으로 기능**할 수 있다.

이 RealFlight Link를 통해, 실제 하드웨어 없이 시뮬레이터로 소프트웨어를 검증하는 방식인 [SITL(Software In The Loop)](https://ardupilot.org/dev/docs/sitl-simulator-software-in-the-loop.html) 구조로 전체 시스템을 구성한다. 에이전트는 시뮬레이터가 보내는 상태를 받아 행동을 결정하고, 그 결과를 다시 조종 입력으로 돌려보낸다.

![SITL 구조를 응용한 시스템 구성](/assets/img/hover-pilot/dev-setup-sketch.png)

## 강화학습이 잘 해낼 수 있을까?

호버링은 불안정한 시스템을 안정되게 유지하는 문제이다. 기울어지는 방향과 정도를 파악하고, 그 불안정성을 상쇄하는 힘을 가하는 과정을 통해 목표한 자세를 유지해야 한다.  
...
이 구조는 강화학습의 문제 정의와 거의 일치한다. 에이전트는 매 순간 상태(**state**)를 관찰하고, 이를 바탕으로 행동(**action**)을 선택하며, 그 결과가 목표에 얼마나 가까운지에 따라 보상(**reward**)을 받는다. 그리고 이 과정을 반복하면서 점점 더 안정적인 정책을 만들어간다. 
...
문제는 탐색 공간이 너무 크다는 점이다. 호버링은 네 개의 제어 입력이 동시에 얽혀 있는 문제라, 처음부터 전체를 학습시키면 거의 아무것도 배우지 못한 채 추락만 반복하게 될 것이다.  
