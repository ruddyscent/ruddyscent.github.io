...
- 의미가 달라지지 않는다면 과거형보다 현재형을 우선한다. 특정 버전의 코드를 설명할 때도 대상 버전을 명시한 뒤 동작은 현재형으로 기술한다.
...
- 의미가 달라지지 않는다면 현재형을 우선합니다. 특정 버전의 구현도 대상 버전을 명시한 뒤 현재형으로 설명하되, 실제 경험이나 시간 순서가 중요한 사건은 과거형을 유지합니다.
