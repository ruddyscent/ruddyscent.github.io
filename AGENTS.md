...
- 실험 결과·스크린샷·영상에 기반한 설명에는 해당 코드의 커밋과 재현에 필요한 실행 명령·환경·주요 설정을 함께 기록한다. 확인되지 않은 실행 조건은 추정하지 않고, 다른 버전에서 얻은 결과를 현재 코드의 결과처럼 설명하지 않는다.
...
- 이미지·캡션·레이아웃을 수정하면 빌드 성공뿐 아니라 작업 worktree의 실제 페이지에서 배치와 표시 상태를 확인한다. 썸네일은 기존 아이콘과 같은 표시 크기로 비교하고, 확인하지 못한 화면은 검증 범위에서 제외했다고 명시한다.

### 이미지 수정

- 채택한 기준 이미지와 유지할 요소를 작업 기록에 남긴다. 후속 수정은 요청된 요소에 집중하고, 수정 전후를 비교해 이미 확정한 모양·배치·투명 배경 등이 훼손되지 않았는지 확인한다. 사용자가 방향을 바꾸면 기준도 갱신한다.
- 개별 썸네일의 크기나 여백 문제는 이미지 자체와 기존 스타일을 먼저 확인한다. 공통 CSS 변경이 필요하면 적용 범위를 좁히고 다른 포스트의 표시에도 문제가 없는지 확인한다. 이미지 수정으로 불필요해진 이번 작업의 임시 CSS 보정은 제거한다.
...
- 미래 날짜 포스트의 포함 여부는 파일 위치만으로 판단하지 않고, 해당 빌드의 설정과 실행 옵션을 확인한다.
- 현재 `.github/workflows/ci.yml`은 `jekyll build --future`로 미래 날짜 포스트도 빌드한다. 이 CI의 통과나 로컬 미리보기만으로 실제 사이트의 공개 상태를 단정하지 않는다.
- 예약 날짜를 수정하면 파일명, frontmatter의 `date`가 있는 경우 그 값, 관련 내부 링크를 함께 확인한다. 공개 여부는 실제 배포 경로의 설정·빌드 시각·시간대를 확인하고, 배포 완료 후 대상 URL에서 검증한다. 이 확인 규칙이 배포 자체를 허가하는 것은 아니다.
